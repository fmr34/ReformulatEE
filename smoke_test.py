...
ok &= check(
    "src.corpus.index import", lambda: __import__("src.corpus.index", fromlist=["CorpusIndex"])
)
ok &= check(
    "src.ee.nao_trivialidade import",
    lambda: __import__("src.ee.nao_trivialidade", fromlist=["nao_trivialidade"]),
)
ok &= check(
    "src.eval.curated import + validacao",
    lambda: __import__("src.eval.curated", fromlist=["get_curated"]).get_curated(),
)
...
ok &= check(
    ...
    lambda: (
        None
        if abs(bell(0.45) - 1.0) < 0.01
        else (_ for _ in ()).throw(AssertionError(f"bell(0.45)={bell(0.45)}"))
    ),
)
ok &= check(
    "bell(0.0) < 0.1",
    lambda: (
        None if bell(0.0) < 0.1 else (_ for _ in ()).throw(AssertionError(f"bell(0.0)={bell(0.0)}"))
    ),
)
ok &= check(
    "bell(1.0) < 0.1",
    lambda: (
        None if bell(1.0) < 0.1 else (_ for _ in ()).throw(AssertionError(f"bell(1.0)={bell(1.0)}"))
    ),
)
